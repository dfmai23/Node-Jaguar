/*******************************************************************************
* File Name: LED_Driver2_MASTER.c
* Version 3.50
*
* Description:
...
volatile uint8 LED_Driver2_mstrStatus;     /* Master Status byte  */
volatile uint8 LED_Driver2_mstrControl;    /* Master Control byte */

/* Transmit buffer variables */
volatile uint8 * LED_Driver2_mstrRdBufPtr;     /* Pointer to Master Read buffer */
volatile uint8   LED_Driver2_mstrRdBufSize;    /* Master Read buffer size       */
volatile uint8   LED_Driver2_mstrRdBufIndex;   /* Master Read buffer Index      */

/* Receive buffer variables */
volatile uint8 * LED_Driver2_mstrWrBufPtr;     /* Pointer to Master Write buffer */
volatile uint8   LED_Driver2_mstrWrBufSize;    /* Master Write buffer size       */
volatile uint8   LED_Driver2_mstrWrBufIndex;   /* Master Write buffer Index      */


/*******************************************************************************
* Function Name: LED_Driver2_MasterWriteBuf
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  LED_Driver2_mstrStatus  - The global variable used to store a current
*                                 status of the I2C Master.
*  LED_Driver2_state       - The global variable used to store a current
*                                 state of the software FSM.
*  LED_Driver2_mstrControl - The global variable used to control the master
*                                 end of a transaction with or without Stop
*                                 generation.
*  LED_Driver2_mstrWrBufPtr - The global variable used to store a pointer
*                                  to the master write buffer.
*  LED_Driver2_mstrWrBufIndex - The global variable used to store current
*                                    index within the master write buffer.
*  LED_Driver2_mstrWrBufSize - The global variable used to store a master
*                                   write buffer size.
*
* Reentrant:
*  No
      ...
      
{
    ...
    errStatus = LED_Driver2_MSTR_NOT_READY;

    if(NULL != wrData)
    {
        /* Check I2C state to allow transfer: valid states are IDLE or HALT */
        if(LED_Driver2_SM_IDLE == LED_Driver2_state)
        {
            /* Master is ready for transaction: check if bus is free */
            if(LED_Driver2_CHECK_BUS_FREE(LED_Driver2_MCSR_REG))
            {
                errStatus = LED_Driver2_MSTR_NO_ERROR;
            }
            else
            {
                errStatus = LED_Driver2_MSTR_BUS_BUSY;
            }
        }
        else if(LED_Driver2_SM_MSTR_HALT == LED_Driver2_state)
        {
            /* Master is ready and waiting for ReStart */
            errStatus = LED_Driver2_MSTR_NO_ERROR;

            LED_Driver2_ClearPendingInt();
            LED_Driver2_mstrStatus &= (uint8) ~LED_Driver2_MSTAT_XFER_HALT;
        }
        else
        {
            /* errStatus = LED_Driver2_MSTR_NOT_READY was send before */
        }

        if(LED_Driver2_MSTR_NO_ERROR == errStatus)
        {
            /* Set state to start write transaction */
            LED_Driver2_state = LED_Driver2_SM_MSTR_WR_ADDR;

            /* Prepare write buffer */
            LED_Driver2_mstrWrBufIndex = 0u;
            LED_Driver2_mstrWrBufSize  = cnt;
            LED_Driver2_mstrWrBufPtr   = (volatile uint8 *) wrData;

            /* Set end of transaction flag: Stop or Halt (following ReStart) */
            LED_Driver2_mstrControl = mode;

            /* Clear write status history */
            LED_Driver2_mstrStatus &= (uint8) ~LED_Driver2_MSTAT_WR_CMPLT;

            /* Hardware actions: write address and generate Start or ReStart */
            LED_Driver2_DATA_REG = (uint8) (slaveAddress << LED_Driver2_SLAVE_ADDR_SHIFT);

            if(LED_Driver2_CHECK_RESTART(mode))
            {
...
/*******************************************************************************
* Function Name: LED_Driver2_MasterReadBuf
********************************************************************************
*
* Summary:
*  Automatically writes an entire buffer of data to a slave device. Once the
...
*                                 Stop generation.
*  LED_Driver2_mstrRdBufPtr - The global variable used to store a pointer
*                                  to the master write buffer.
*  LED_Driver2_mstrRdBufIndex - The global variable  used to store a
*                                    current index within the master
*                                    write buffer.
*  LED_Driver2_mstrRdBufSize - The global variable used to store a master
*                                   write buffer size.
*
* Reentrant:
*  No.
      ...
      
{
    ...
    if(NULL != rdData)
    {
        ...
        {
            /* errStatus = LED_Driver2_MSTR_NOT_READY was set before */
        }

        if(LED_Driver2_MSTR_NO_ERROR == errStatus)
        {
            /* Set state to start write transaction */
            LED_Driver2_state = LED_Driver2_SM_MSTR_RD_ADDR;

            /* Prepare read buffer */
            LED_Driver2_mstrRdBufIndex  = 0u;
            LED_Driver2_mstrRdBufSize   = cnt;
            LED_Driver2_mstrRdBufPtr    = (volatile uint8 *) rdData;

            /* Set end of transaction flag: Stop or Halt (following ReStart) */
            LED_Driver2_mstrControl = mode;

            /* Clear read status history */
            LED_Driver2_mstrStatus &= (uint8) ~LED_Driver2_MSTAT_RD_CMPLT;

            /* Hardware actions: write address and generate Start or ReStart */
            LED_Driver2_DATA_REG = ((uint8) (slaveAddress << LED_Driver2_SLAVE_ADDR_SHIFT) |
                                                  LED_Driver2_READ_FLAG);
...
/*******************************************************************************
* Function Name: LED_Driver2_MasterSendStart
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  LED_Driver2_state - The global variable used to store a current state of
*                           the software FSM.
*
* Reentrant:
*  No.
      ...
      
{
    ...
    if(LED_Driver2_SM_IDLE == LED_Driver2_state)
    {
        ...
        if(LED_Driver2_CHECK_BUS_FREE(LED_Driver2_MCSR_REG))
        {
            ...
            slaveAddress = (uint8) (slaveAddress << LED_Driver2_SLAVE_ADDR_SHIFT);
            if(0u != R_nW)
            {
                slaveAddress |= LED_Driver2_READ_FLAG;
                LED_Driver2_state = LED_Driver2_SM_MSTR_RD_ADDR;
            }
            else
            {
            ...
            LED_Driver2_DATA_REG = slaveAddress;
            LED_Driver2_GENERATE_START_MANUAL;

            /* Wait until address is transferred */
            while(LED_Driver2_WAIT_BYTE_COMPLETE(LED_Driver2_CSR_REG))
            {
            }
            ...
            if(LED_Driver2_CHECK_START_GEN(LED_Driver2_MCSR_REG))
            {
                ...
                errStatus = LED_Driver2_MSTR_ERR_ABORT_START_GEN;
            }
            else
        #endif /* (LED_Driver2_MODE_MULTI_MASTER_SLAVE_ENABLED) */

        #if(LED_Driver2_MODE_MULTI_MASTER_ENABLED)
            if(LED_Driver2_CHECK_LOST_ARB(LED_Driver2_CSR_REG))
            {
                LED_Driver2_BUS_RELEASE_MANUAL;

                /* Master lost arbitrage: reset FSM to IDLE */
                LED_Driver2_state = LED_Driver2_SM_IDLE;
                errStatus = LED_Driver2_MSTR_ERR_ARB_LOST;
            }
            else
        #endif /* (LED_Driver2_MODE_MULTI_MASTER_ENABLED) */

            if(LED_Driver2_CHECK_ADDR_NAK(LED_Driver2_CSR_REG))
            {
                /* Address has been NACKed: reset FSM to IDLE */
                LED_Driver2_state = LED_Driver2_SM_IDLE;
                errStatus = LED_Driver2_MSTR_ERR_LB_NAK;
            }
            else
            {
...
/*******************************************************************************
* Function Name: LED_Driver2_MasterSendRestart
********************************************************************************
*
* Summary:
      ...
      
{
    ...
    if(LED_Driver2_CHECK_MASTER_MODE(LED_Driver2_MCSR_REG))
    {
        /* Set address and read/write flag */
        slaveAddress = (uint8) (slaveAddress << LED_Driver2_SLAVE_ADDR_SHIFT);
        if(0u != R_nW)
        ...
        LED_Driver2_DATA_REG = slaveAddress;
        LED_Driver2_GENERATE_RESTART_MANUAL;
...
/*******************************************************************************
* Function Name: LED_Driver2_MasterSendStop
********************************************************************************
*
* Summary:
...
uint8 LED_Driver2_MasterSendStop(void) 
{
    ...
    if(LED_Driver2_CHECK_MASTER_MODE(LED_Driver2_MCSR_REG))
    {
        LED_Driver2_GENERATE_STOP_MANUAL;
        LED_Driver2_state = LED_Driver2_SM_IDLE;

        /* Wait until stop has been generated */
        while(LED_Driver2_WAIT_STOP_COMPLETE(LED_Driver2_CSR_REG))
        {
        }
...
/*******************************************************************************
* Function Name: LED_Driver2_MasterWriteByte
********************************************************************************
*
* Summary:
...
uint8 LED_Driver2_MasterWriteByte(uint8 theByte) 
{
    ...
    if(LED_Driver2_CHECK_MASTER_MODE(LED_Driver2_MCSR_REG))
    {
        LED_Driver2_DATA_REG = theByte;   /* Write DATA register */
        LED_Driver2_TRANSMIT_DATA_MANUAL; /* Set transmit mode   */
        ...
        if(LED_Driver2_CHECK_DATA_ACK(LED_Driver2_CSR_REG))
        {
            LED_Driver2_state = LED_Driver2_SM_MSTR_HALT;
            errStatus = LED_Driver2_MSTR_NO_ERROR;
        }
        else
        {
...
/*******************************************************************************
* Function Name: LED_Driver2_MasterReadByte
********************************************************************************
*
* Summary:
...
uint8 LED_Driver2_MasterReadByte(uint8 acknNak) 
{
    ...
    if(LED_Driver2_CHECK_MASTER_MODE(LED_Driver2_MCSR_REG))
    {
        ...
        */
        if(LED_Driver2_SM_MSTR_RD_ADDR == LED_Driver2_state)
        {
            LED_Driver2_READY_TO_READ_MANUAL;
        ...
        */
        ...
        {
            LED_Driver2_ACK_AND_RECEIVE_MANUAL;
        }
...
/*******************************************************************************
* Function Name: LED_Driver2_MasterStatus
********************************************************************************
*
* Summary:
...
uint8 LED_Driver2_MasterStatus(void) 
{
    ...
    LED_Driver2_DisableInt(); /* Lock from interrupt */

    /* Read master status */
    status = LED_Driver2_mstrStatus;

    if (LED_Driver2_CHECK_SM_MASTER)
    {
        /* Set transfer in progress flag in status */
        status |= LED_Driver2_MSTAT_XFER_INP;
    }

    LED_Driver2_EnableInt(); /* Release lock */
...
/*******************************************************************************
* Function Name: LED_Driver2_MasterClearStatus
********************************************************************************
*
* Summary:
...
uint8 LED_Driver2_MasterClearStatus(void) 
{
    ...
    status = LED_Driver2_mstrStatus;
    LED_Driver2_mstrStatus = LED_Driver2_MSTAT_CLEAR;
...
/*******************************************************************************
* Function Name: LED_Driver2_MasterGetReadBufSize
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  LED_Driver2_mstrRdBufIndex - The global variable stores current index
*                                    within the master read buffer.
*
*******************************************************************************/
uint8 LED_Driver2_MasterGetReadBufSize(void) 
{
    return (LED_Driver2_mstrRdBufIndex);
}


/*******************************************************************************
* Function Name: LED_Driver2_MasterGetWriteBufSize
********************************************************************************
*
* Summary:
*  Returns the amount of bytes that has been transferred with an
...
*
* Global variables:
*  LED_Driver2_mstrWrBufIndex -  The global variable used to stores current
*                                     index within master write buffer.
*
*******************************************************************************/
uint8 LED_Driver2_MasterGetWriteBufSize(void) 
{
    return (LED_Driver2_mstrWrBufIndex);
}


/*******************************************************************************
* Function Name: LED_Driver2_MasterClearReadBuf
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  LED_Driver2_mstrRdBufIndex - The global variable used to stores current
...
/*******************************************************************************
* Function Name: LED_Driver2_MasterClearWriteBuf
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  LED_Driver2_mstrRdBufIndex - The global variable used to stote current
*                                    index within master read buffer.
*  LED_Driver2_mstrStatus - The global variable used to store a current
*                                status of the I2C Master.
*
