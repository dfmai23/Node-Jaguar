...
LED_Driver2_BACKUP_STRUCT LED_Driver2_backup =
{
    LED_Driver2_DISABLE,

#if (LED_Driver2_FF_IMPLEMENTED)
    LED_Driver2_DEFAULT_XCFG,
    LED_Driver2_DEFAULT_CFG,
    LED_Driver2_DEFAULT_ADDR,
    LO8(LED_Driver2_DEFAULT_DIVIDE_FACTOR),
    HI8(LED_Driver2_DEFAULT_DIVIDE_FACTOR),
#else  /* (LED_Driver2_UDB_IMPLEMENTED) */
    LED_Driver2_DEFAULT_CFG,
#endif /* (LED_Driver2_FF_IMPLEMENTED) */

#if (LED_Driver2_TIMEOUT_ENABLED)
    LED_Driver2_DEFAULT_TMOUT_PERIOD,
    LED_Driver2_DEFAULT_TMOUT_INTR_MASK,
#endif /* (LED_Driver2_TIMEOUT_ENABLED) */
};

#if ((LED_Driver2_FF_IMPLEMENTED) && (LED_Driver2_WAKEUP_ENABLED))
    volatile uint8 LED_Driver2_wakeupSource;
#endif /* ((LED_Driver2_FF_IMPLEMENTED) && (LED_Driver2_WAKEUP_ENABLED)) */


/*******************************************************************************
* Function Name: LED_Driver2_SaveConfig
********************************************************************************
*
* Summary:
...
*
* Global Variables:
*  LED_Driver2_backup - The global variable used to save the component
...
{
#if (LED_Driver2_FF_IMPLEMENTED)
    #if (LED_Driver2_WAKEUP_ENABLED)
        uint8 intState;
    #endif /* (LED_Driver2_WAKEUP_ENABLED) */

    /* Store registers before enter low power mode */
    LED_Driver2_backup.cfg     = LED_Driver2_CFG_REG;
    LED_Driver2_backup.xcfg    = LED_Driver2_XCFG_REG;
...
#if (LED_Driver2_WAKEUP_ENABLED)
    /* Disable master */
    LED_Driver2_CFG_REG &= (uint8) ~LED_Driver2_ENABLE_MASTER;

    /* Enable backup regulator to keep block powered in low power mode */
    intState = CyEnterCriticalSection();
    LED_Driver2_PWRSYS_CR1_REG |= LED_Driver2_PWRSYS_CR1_I2C_REG_BACKUP;
    ...
    */
    ...
    LED_Driver2_XCFG_REG |= LED_Driver2_XCFG_FORCE_NACK;
    while (0u == (LED_Driver2_XCFG_REG & LED_Driver2_XCFG_RDY_TO_SLEEP))
    {
    }

    /* Setup wakeup interrupt */
    LED_Driver2_DisableInt();
    (void) CyIntSetVector(LED_Driver2_ISR_NUMBER, &LED_Driver2_WAKEUP_ISR);
    LED_Driver2_wakeupSource = 0u;
    LED_Driver2_EnableInt();
#endif /* (LED_Driver2_WAKEUP_ENABLED) */

#else
    /* Store only address match bit */
    LED_Driver2_backup.control = (LED_Driver2_CFG_REG & LED_Driver2_CTRL_ANY_ADDRESS_MASK);
#endif /* (LED_Driver2_FF_IMPLEMENTED) */

#if (LED_Driver2_TIMEOUT_ENABLED)
    LED_Driver2_TimeoutSaveConfig();
#endif /* (LED_Driver2_TIMEOUT_ENABLED) */
}


/*******************************************************************************
* Function Name: LED_Driver2_Sleep
********************************************************************************
*
* Summary:
...
void LED_Driver2_Sleep(void) 
{
    ...
    LED_Driver2_backup.enableState = LED_Driver2_DISABLE;
...
#else
    /* Store enable state */
    LED_Driver2_backup.enableState = (uint8) LED_Driver2_IS_ENABLED;

    if (0u != LED_Driver2_backup.enableState)
    {
        LED_Driver2_Stop();
    }
#endif /* (LED_Driver2_WAKEUP_ENABLED) */

    LED_Driver2_SaveConfig();
}


/*******************************************************************************
* Function Name: LED_Driver2_RestoreConfig
********************************************************************************
*
* Summary:
*  The Enable wakeup from Sleep Mode selection influences this function
...
*
* Side Effects:
*  Calling this function before LED_Driver2_SaveConfig() or
*  LED_Driver2_Sleep() will lead to unpredictable results.
*
*******************************************************************************/
void LED_Driver2_RestoreConfig(void) 
{
    ...
    if (LED_Driver2_CHECK_PWRSYS_I2C_BACKUP)
    /* Low power mode was Sleep - backup regulator is enabled */
    {
        /* Enable backup regulator in active mode */
        intState = CyEnterCriticalSection();
        LED_Driver2_PWRSYS_CR1_REG &= (uint8) ~LED_Driver2_PWRSYS_CR1_I2C_REG_BACKUP;
        CyExitCriticalSection(intState);

        /* Restore master */
        LED_Driver2_CFG_REG = LED_Driver2_backup.cfg;
    }
    else
    /* Low power mode was Hibernate - backup regulator is disabled. All registers are cleared */
    {
        ...
        LED_Driver2_backup.enableState = LED_Driver2_ENABLE;
    #endif /* (LED_Driver2_WAKEUP_ENABLED) */

        /* Restore component registers after Hibernate */
        LED_Driver2_XCFG_REG    = LED_Driver2_backup.xcfg;
        LED_Driver2_CFG_REG     = LED_Driver2_backup.cfg;
    ...
    if (0u != LED_Driver2_wakeupSource)
    {
        /* Generate interrupt to process incoming transaction */
        LED_Driver2_SetPendingInt();
    }
    LED_Driver2_EnableInt();
#endif /* (LED_Driver2_WAKEUP_ENABLED) */

#else
    LED_Driver2_CFG_REG = LED_Driver2_backup.control;
#endif /* (LED_Driver2_FF_IMPLEMENTED) */

#if (LED_Driver2_TIMEOUT_ENABLED)
    LED_Driver2_TimeoutRestoreConfig();
#endif /* (LED_Driver2_TIMEOUT_ENABLED) */
}


/*******************************************************************************
* Function Name: LED_Driver2_Wakeup
********************************************************************************
*
* Summary:
