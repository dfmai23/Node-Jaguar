...
volatile uint8 LED_Driver2_slStatus;   /* Slave Status  */

/* Transmit buffer variables */
volatile uint8 * LED_Driver2_slRdBufPtr;   /* Pointer to Transmit buffer  */
volatile uint8   LED_Driver2_slRdBufSize;  /* Slave Transmit buffer size  */
volatile uint8   LED_Driver2_slRdBufIndex; /* Slave Transmit buffer Index */

/* Receive buffer variables */
volatile uint8 * LED_Driver2_slWrBufPtr;   /* Pointer to Receive buffer  */
volatile uint8   LED_Driver2_slWrBufSize;  /* Slave Receive buffer size  */
volatile uint8   LED_Driver2_slWrBufIndex; /* Slave Receive buffer Index */

#if (LED_Driver2_SW_ADRR_DECODE)
    volatile uint8 LED_Driver2_slAddress;  /* Software address variable */
#endif /* (LED_Driver2_SW_ADRR_DECODE) */


/*******************************************************************************
* Function Name: LED_Driver2_SlaveStatus
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  LED_Driver2_slStatus - The global variable used to store a current
*                              status of the I2C slave.
*
*******************************************************************************/
uint8 LED_Driver2_SlaveStatus(void) 
{
    return (LED_Driver2_slStatus);
}


/*******************************************************************************
* Function Name: LED_Driver2_SlaveClearReadStatus
********************************************************************************
*
* Summary:
*  Clears the read status flags and returns they values.
*  The LED_Driver2_SSTAT_RD_BUSY flag is not effected by clear.
*
* Parameters:
*  None.
*
...
uint8 LED_Driver2_SlaveClearReadStatus(void) 
{
    ...
    status = (LED_Driver2_slStatus & LED_Driver2_SSTAT_RD_MASK);
    LED_Driver2_slStatus &= (uint8) ~LED_Driver2_SSTAT_RD_CLEAR;
...
/*******************************************************************************
* Function Name: LED_Driver2_SlaveClearWriteStatus
********************************************************************************
*
* Summary:
*  Clears the write status flags and returns they values.
*  The LED_Driver2_SSTAT_WR_BUSY flag is not effected by clear.
*
* Parameters:
*  None.
*
...
uint8 LED_Driver2_SlaveClearWriteStatus(void) 
{
    ...
    /* Mask of transfer complete flag and Error status */
    status = (LED_Driver2_slStatus & LED_Driver2_SSTAT_WR_MASK);
    LED_Driver2_slStatus &= (uint8) ~LED_Driver2_SSTAT_WR_CLEAR;
...
/*******************************************************************************
* Function Name: LED_Driver2_SlaveSetAddress
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  LED_Driver2_Address  - The global variable used to store an I2C slave
...
{
#if (LED_Driver2_SW_ADRR_DECODE)
    LED_Driver2_slAddress = (address & LED_Driver2_SLAVE_ADDR_MASK);
#else
    LED_Driver2_ADDR_REG  = (address & LED_Driver2_SLAVE_ADDR_MASK);
#endif /* (LED_Driver2_SW_ADRR_DECODE) */
}


/*******************************************************************************
* Function Name: LED_Driver2_SlaveInitReadBuf
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  LED_Driver2_slRdBufPtr   - The global variable used to store a pointer
*                                  to the slave read buffer.
*  LED_Driver2_slRdBufSize  - The global variable used to store a slave
*                                  read buffer size.
*  LED_Driver2_slRdBufIndex - The global variable used to store a current
     ...
     
{
    if (NULL != rdBuf)
    {
        ...
        /* Set buffer pointer */
        LED_Driver2_slRdBufPtr   = (volatile uint8 *) rdBuf;
        LED_Driver2_slRdBufSize  = bufSize;    /* Set buffer size     */
        LED_Driver2_slRdBufIndex = 0u;         /* Clears buffer index */
...
/*******************************************************************************
* Function Name: LED_Driver2_SlaveInitWriteBuf
********************************************************************************
*
* Summary:
*  Sets the buffer pointer and size of the read buffer. This function also
...
*
* Global variables:
*  LED_Driver2_slWrBufPtr   - The global variable used to store a pointer
*                                  to the slave write buffer.
*  LED_Driver2_slWrBufSize  - The global variable used to store a slave
*                                  write buffer size.
*  LED_Driver2_slWrBufIndex - The global variable used to store a current
     ...
     
{
    if (NULL != wrBuf)
    {
        LED_Driver2_DisableInt(); /* Lock from interrupt */
         ...
         /* Set buffer pointer */
        LED_Driver2_slWrBufPtr   = (volatile uint8 *) wrBuf;
        LED_Driver2_slWrBufSize  = bufSize;    /* Set buffer size     */
        LED_Driver2_slWrBufIndex = 0u;         /* Clears buffer index */
...
/*******************************************************************************
* Function Name: LED_Driver2_SlaveGetReadBufSize
********************************************************************************
*
* Summary:
...
{
    return (LED_Driver2_slRdBufIndex);
}


/*******************************************************************************
* Function Name: LED_Driver2_SlaveGetWriteBufSize
********************************************************************************
*
* Summary:
...
{
    return (LED_Driver2_slWrBufIndex);
}


/*******************************************************************************
* Function Name: LED_Driver2_SlaveClearReadBuf
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: LED_Driver2_SlaveClearRxBuf
********************************************************************************
*
* Summary:
